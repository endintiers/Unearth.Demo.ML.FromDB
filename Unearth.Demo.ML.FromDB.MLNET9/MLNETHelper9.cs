...
using Unearth.Demo.ML.FromDB.MLNET9.Models;

namespace Unearth.Demo.ML.FromDB.MLNET9
{
    public class MLNETHelper9
    {
        public static ITransformer TrainModel(string connectionString = null, int nth = 1, int threads = -1)
        {
            ...
            using (var airlinesModel = new AirlinesContext(optionsBuilder.Options))
            {
                ...
                Console.WriteLine($"Training the ML.NET 0.9 model with {threads} threads... ");
                ...
                if (threads > 0)
                {
                    trainer = mlContext.MulticlassClassification.Trainers.StochasticDualCoordinateAscent(
                        ...
                        advancedSettings: (s) => s.NumThreads = threads);
                }
                else
                {
                    trainer = mlContext.MulticlassClassification.Trainers.StochasticDualCoordinateAscent(
